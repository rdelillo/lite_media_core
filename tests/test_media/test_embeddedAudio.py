...
class TestEmbeddedAudio(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		cls.embeddedAudio = media.EmbeddedAudio(cls.url)

	def test_valid_embedded(self):
		""" Ensure an embedded media can be created from a valid url.
		"""
		self.assertIsInstance(self.embeddedAudio, media.EmbeddedAudio)
	...
	def test_settings(self):
		...
		self.assertEqual(
			...
			(
				self.embeddedAudio.duration,
				str(self.embeddedAudio.conformedDuration),
				self.embeddedAudio.bitrate,
				self.embeddedAudio.samplingRate,
			)
		)
